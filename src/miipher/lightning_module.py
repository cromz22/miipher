...
from lightning_vocoders.models.hifigan.lightning_module import (
...
class MiipherLightningModule(LightningModule):
    ...
    def configure_optimizers(self):
        return hydra.utils.instantiate(
            self.cfg.optimizers, params=self.miipher.parameters()
        )
    ...
    def criterion(
        ...
        for idx, intermediate in enumerate(intermediates):
            ...
            spectoral_loss = (
                (intermediate - target + 1e-7).norm(p=2, dim=(1, 2)).pow(2)
                / (target.norm(p=2, dim=(1, 2)).pow(2))
            ).mean()
    ...
    def synthesis(self, features: torch.Tensor, wav16k, wav16k_lens):
        ...
        return vocoder.generator_forward(
            {
                "input_feature": features.unsqueeze(0).cpu().float(),
                "xvector": xvector.cpu().float(),
            }
        )[0].T
